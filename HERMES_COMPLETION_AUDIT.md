...
- **Current local validation:** 242 package tests pass with warnings-as-errors; 97 integrated macOS app tests pass; seven Python release/security tests, deterministic project/assets, privacy checks, and the generic dual-architecture iOS Simulator test build pass for build 6 candidate source `f44a3ad`.
...
| Hermes Agent | Runs, jobs, streaming, approvals, schedules, provenance | Typed run/status/approval/stop contracts, app-wired read-only review over bounded incremental URLSession SSE, encrypted durable one-shot operation evidence, fail-closed human review, and an executable 11-category evidence qualifier currently at 0/11 | Verified authorization/live mutation composition, reconnect/cursor contract, and sanctioned live private-service evidence |
| War Room | Fleet, Mesh, routes, alerts; some mock data | Credential- and canonical-MeshID-gated read-only Mesh dashboard with provenance/partial states and an executable 10-category evidence qualifier currently at 0/10 | Sanctioned private gateway capture and device/Mac runtime evidence |
...
- Encrypted multi-workspace catalog with explicit active selection, deterministic verified fallback after active-workspace deletion, and isolated credential/profile erasure.
