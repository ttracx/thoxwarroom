...
- **Native provider surfaces** — Open WebUI exposes bounded public discovery and a protected model catalog; its offline evidence qualifier keeps native chat/stream/citation capabilities disabled until all nine authenticated contract categories are captured, sanitized, and reviewed. Hermes concurrently loads status and a live incremental URLSession SSE stream into a bounded read-only review, with explicit completion/error/cancellation states. Opt-in DNS-bound Network.framework provider and Hermes SSE transports are source-complete and independently tested with numeric-peer binding, original-host TLS identity, per-hop re-resolution, bounded HTTP/1.1 framing, and exactly-once terminal cancellation, but they are not yet app-composed or live/physical-runtime-qualified. A one-shot coordinator and encrypted device-local operation store durably claim intent, reject replay across workspaces, record bounded outcomes, detect rollback/deletion, and expose pending/terminal crash reconciliation without retrying transport. A separate fixed-root evidence qualifier currently reports 0 of 11 live Hermes categories captured. The human-review UI is implemented but production composition remains fail-closed until workspace authorization and a current live mutation contract are verified.
