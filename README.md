...
> **Current scope:** v4.2 has the clean-room shared core, encrypted workspace-profile persistence with resumable credential-first deletion, a crash-safe rollback-anchored audit lifecycle with bounded redacted export, an audited Hermes mutation coordinator seam, native provider selection, Open WebUI discovery/credential/model-catalog UI, a credential-gated read-only Hermes run review with incremental URLSession SSE delivery, a credential-gated read-only MeshStack War Room dashboard, an explicitly selected confined local text browser, and native macOS workspace navigation. Native authenticated chat, production mutation-store/App wiring, encrypted chat/document history, live private-service verification, and physical-device proof remain unfinished. Both iOS and native macOS build 2 artifacts are valid in TestFlight and assigned to the THOX internal group; invited-tester install/launch is still unobserved. See the [Hermes completion audit](HERMES_COMPLETION_AUDIT.md), [current service contracts](docs/current_service_contracts.md), [MVP catalog](mvp_catalog.md), and [multi-team development queue](development_queue.md).
...
- **Native provider surfaces** — Open WebUI exposes bounded public discovery and a protected model catalog; its executable evidence gate keeps native chat/stream/citation capabilities disabled until authenticated contracts are captured. Hermes concurrently loads status and a live incremental URLSession SSE stream into a bounded read-only review, with explicit completion/error/cancellation states. A transport-neutral audited operation coordinator now claims mutation intent before transport and records bounded outcomes without retrying; production persistence/App controls, reconnect/cursor semantics, and live-provider evidence remain unavailable.
- **Encrypted durable audit lifecycle** — workspace-scoped AES-GCM ledgers revalidate redaction, chain ordered entries, bind cursors to workspace/digest, and bound capacity/pages. Device-only Keychain generations reject rollback while allowing crash-safe retention compaction. Retention supports 30–2555 days (365 default) or explicit indefinite storage, and export is bounded, redacted, integrity-verified, and in-memory only. App policy scheduling/UI and external non-repudiation remain gaps.
