...
- **Native provider surfaces** — Open WebUI exposes bounded public discovery and a protected model catalog; its offline evidence qualifier keeps native chat/stream/citation capabilities disabled until all nine authenticated contract categories are captured, sanitized, and reviewed. Current app source composes independently reviewed DNS-bound Network.framework request/response and Hermes SSE transports for HTTPS workspaces, with numeric-peer binding, original-host TLS identity, per-hop re-resolution, bounded HTTP/1.1 framing, cancellation-aware DNS, activity/resource timeouts, and exactly-once terminal cancellation. Only explicitly validated loopback HTTP retains URLSession compatibility; legacy private/hosted HTTP fails closed. This source composition is not yet present in a signed build or live/physical-runtime-qualified. A one-shot coordinator and encrypted device-local operation store durably claim intent, reject replay across workspaces, record bounded outcomes, detect rollback/deletion, and expose pending/terminal crash reconciliation without retrying transport. A separate fixed-root evidence qualifier currently reports 0 of 11 live Hermes categories captured. The human-review UI is implemented but mutation composition remains fail-closed until workspace authorization and a current live mutation contract are verified.
