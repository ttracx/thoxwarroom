...
> **Current scope:** v4.2 has the clean-room shared core, encrypted workspace-profile persistence with resumable credential-first deletion, a crash-safe rollback-anchored audit lifecycle, encrypted workspace audit-policy persistence, explicit foreground-only retention application, bounded in-memory redacted export through the system destination picker, encrypted durable one-shot Hermes operation evidence with replay and crash reconciliation, a fail-closed human mutation-review surface, native provider selection, Open WebUI discovery/credential/model-catalog UI, a credential-gated read-only Hermes run review with incremental URLSession SSE delivery, a credential-gated read-only MeshStack War Room dashboard, an explicitly selected confined local text browser, and native macOS workspace navigation. Native authenticated chat, verified Hermes authorization/live mutation composition, encrypted chat/document history, live private-service verification, and physical-device proof remain unfinished. Both iOS and native macOS build 3 artifacts are Apple `VALID`, `IN_BETA_TESTING`, and assigned to the THOX internal group; build 4 delivery is the next signed release lane and invited-tester install/launch is still unobserved. See the [Hermes completion audit](HERMES_COMPLETION_AUDIT.md), [current service contracts](docs/current_service_contracts.md), [MVP catalog](mvp_catalog.md), and [multi-team development queue](development_queue.md).
