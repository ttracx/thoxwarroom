...
## ADR-022: Land DNS-bound transports as opt-in infrastructure before app adoption

- **Decision:** Land separately injectable Network.framework transports for bounded provider request/response and Hermes SSE traffic, while retaining the existing URLSession transports in production composition until live and physical-runtime qualification succeeds.
...
- **Options considered:** replace URLSession immediately; keep only the feasibility spike; add preflight DNS checks around URLSession; land opt-in implementations behind existing transport protocols and qualify them before composition.
- **Tradeoffs:** the opt-in slice adds parallel transport code and delays user-visible adoption, but preserves working behavior while allowing focused source, security, live-network, and physical-device evidence to be gathered independently.
...
- **Compliance impact:** independently tested infrastructure improves auditability but is not evidence of live provider authorization, private-CA compatibility, physical-device behavior, or deployed DNS-rebinding resistance.
- **Final choice:** retain both DNS-bound transports as explicit dependencies, record their source-only status, and prohibit app readiness claims until composition and runtime gates pass.
- **Follow-up:** complete integration threat review, compose behind an explicit workspace policy without weakening existing evidence gates, then verify sanctioned private/public certificate paths and cancellation/redirect behavior on TestFlight iOS and macOS builds.
