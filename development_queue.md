...
| WR-003 | 8.8 | Encryption/migration, rollback-anchored audit, cross-process serialization, and resumable deletion implemented; policy wiring pending | Core + Apple | Encrypted profile/audit stores; Keychain anchor/lock/journal; onboarding lifecycle | Ciphertext isolation, redaction, tamper/rollback detection, cooperative writer serialization, bounded pages, credential-first resumable erasure | 31 Core + 72 Apple infrastructure tests; integrated app pipeline pass | Device-only keys, complete iOS protection, backup exclusion; advisory cooperative locks, raw UUID routing, no retention/export or mutation wiring | Add retention/export, background recovery, opaque routing, and audited-operation composition |
| WR-004 | 8.5 | Discovery/model adapter and executable fail-closed chat evidence gate implemented | Core + iOS + macOS | `Packages/WarRoomOpenWebUI`; native chat and streaming | Public discovery/model catalog are bounded; chat capability remains disabled until authenticated request/response/stream/history/citation evidence exists | 16 offline adapter/evidence-gate tests; live UI/reconnect/cancellation pending | Sanitized `401` route evidence cannot enable chat; no guessed DTOs | Provision sanctioned non-production identity and capture authenticated contract evidence |
| WR-005 | 8.7 | App-wired read-only Hermes live review implemented; actions pending | Hermes + Security | `Packages/WarRoomHermes`; Apple URLSession streaming transport; `App/HermesRunReview*`; `App/WorkspaceFeatureHosts.swift` | Typed runs/status/events/stop/approval; bounded incremental SSE, exact-origin redirects, cancellation, cross-run rejection, terminal mapping; read-only UI | 23 Hermes tests plus URLSession/app model tests; live/reconnect/replay/injection pending | No mutation controls; no invented cursor semantics; URLProtocol evidence is not live-provider proof | Capture live private-service evidence and define reconnect/cursor contract before approval integration |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI, SPDX generator | Every PR regenerates deterministically, inventories dependencies, tests every package with warnings as errors, and builds/tests both targets without release secrets | 155 package tests, 71 integrated macOS tests, 3 SPDX tests, deterministic assets/project, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; remote jobs stop before all steps with the account-lock annotation |
...
| WR-013 | 8.6 | Implemented; ASC record pending | iOS + Release | `Assets.xcassets`, privacy manifest, iOS data-protection entitlement, validators, upload script | Catalog/privacy/entitlement validate; signed archive and IPA export succeed | Validators, generic Simulator test build, archive/IPA signature and provisioning verified; encrypted-storage revision needs fresh signed archive | No tracking/collection declared; UserDefaults only selects active workspace; API key remains external | Create the App Store Connect app record for `ai.thox.warroom`, then create a fresh signed archive, upload, and verify processing/install |
...
| WR-015 | 8.8 | Mac App Store archive/export proven; record pending | macOS + Release | `scripts/build_macos_appstore.sh`, SPDX SBOM, metadata worksheet | Automatic App Store signing, sandbox/no-debug validation, signed `.pkg`, optional API-key upload | Apple Distribution app and 3rd Party Mac Developer Installer package verified; upload reaches ASC lookup | API key remains external; no Developer ID/notary claim | Create the shared iOS/macOS app record, rebuild from final SHA, upload, verify processing/install |
