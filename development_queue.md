...
| WR-003 | 8.8 | Profile encryption/migration and bounded encrypted audit ledger implemented; rollback/policy wiring pending | Core + Apple | `EncryptedWorkspaceData`; `EncryptedDurableAuditEventStore`; Apple AES-GCM/Keychain file store; encrypted onboarding migration | Profile/audit ciphertext isolation, redaction revalidation, internal tamper chain, idempotent append, bounded pages, and credential-first erasure pass | 29 Core + 38 Apple infrastructure tests; integrated app pipeline pass | Device-only keys, complete iOS protection, backup exclusion; no Keychain head anchor/CAS, raw UUID routing, retention/export, or app mutation wiring | Add monotonic Keychain anchor, cross-instance serialization, retention/export, and resumable deletion journal |
...
| WR-005 | 8.7 | Read-only buffered Hermes review and incremental stream seam implemented; app wiring/actions pending | Hermes + Security | `Packages/WarRoomHermes`; `App/HermesRunReview*`; `App/WorkspaceFeatureHosts.swift` | Typed runs/status/events/stop/approval; bounded live-byte SSE, cancellation, cross-run rejection; read-only UI | 23 package tests plus app model tests; live/reconnect/replay/injection pending | No mutation controls; stream transport must cancel network task; no invented cursor semantics | Add concrete Apple streaming transport/reconnect contract and live private-service evidence before approval integration |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI | Every PR regenerates deterministically, tests every package with warnings as errors, and builds/tests both targets without release secrets | 119 package tests, integrated macOS tests, deterministic assets/project, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; remote jobs stop before all steps with the account-lock annotation |
