...
| WR-003 | 8.8 | Profile encryption and migration implemented; durable audit pending | Core + Apple | `EncryptedWorkspaceData`, Apple AES-GCM/Keychain file store, encrypted onboarding migration | Profile ciphertext and key isolation pass; legacy data survives failed migration; credentials delete before cryptographic erasure | Core bounds/redaction, codec tamper/wrong-key, atomic/corruption/isolation, onboarding migration tests | Device-only keys, complete iOS file protection, backup exclusion; raw UUID routing and rollback anchors remain gaps | Implement keyed append-only durable audit with rollback anchor, retention, export, and resumable deletion journal |
...
| WR-005 | 8.7 | Credential-gated read-only buffered Hermes review implemented; live stream/actions pending | Hermes + Security | `Packages/WarRoomHermes`; `App/HermesRunReview*`; `App/WorkspaceFeatureHosts.swift` | Typed runs/status/events/stop/approval; read-only UI is available; UI confirmation, durable pre-send audit, replay rejection, and live stream remain | 18 offline package tests plus 7 model tests; live/replay/injection pending | No mutation controls exposed; opaque run IDs and non-sensitive errors | Add streaming transport seam and live private-service evidence before approval integration |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI | Every PR regenerates deterministically, tests every package with warnings as errors, and builds/tests both targets without release secrets | 77 package tests, 58 macOS tests, deterministic assets/project, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; remote jobs stop before all steps with the account-lock annotation |
