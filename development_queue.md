...
| WR-013 | 8.6 | iOS and macOS TestFlight build 4 valid and internally testing; physical install pending | iOS + macOS + Release | App Store Connect record, `Assets.xcassets`, privacy/export declarations, validators, upload scripts | Catalog/privacy/entitlement validate; both signed artifacts process as valid and are assigned to an internal group | iOS/macOS build 4 for version 4.2.0 are `VALID`, `IN_BETA_TESTING`, and externally ready for beta submission; THOX admin invited | API key remains external; source Info.plists declare no non-exempt encryption; direct notarized DMG is separate | Accept the tester invitation and complete physical iPhone/Mac install, launch, authentication, relaunch, and removal checks |
...
| WR-015 | 8.8 | Shared record and native macOS build 4 TestFlight delivery proven; physical install pending | macOS + Release | `scripts/build_macos_appstore.sh`, SPDX SBOM, metadata worksheet, App Store Connect record | Automatic App Store signing, sandbox/no-debug validation, signed `.pkg`, API-key upload and internal assignment | Apple Distribution app and 3rd Party Mac Developer Installer package verified; build 4 reached `VALID` / `IN_BETA_TESTING` | API key remains external; no Developer ID/notary or physical-install claim | Complete invited-tester Mac install/launch/relaunch checks, then run the separate THOX Developer ID notarization lane when credentials exist |
