...
| WR-016 | 9.3 | Opt-in source slice implemented and independently security-reviewed; app composition/live proof pending | Core + Hermes + Security | DNS-bound provider/Hermes transports, HTTP/1.1 codec, DNS classifier, terminal coordinator | Every redirect re-resolves and validates the complete address set; numeric peer retains original-host SNI/default trust; credentials follow exact origin only after validation; framing, buffering, DNS planning, inactivity timeout, and cancellation fail closed | 164 Apple-infrastructure tests pass with warnings as errors; 30 consecutive current full runs (4,920 executions) passed after timeout hardening, plus the earlier 40-run deterministic-barrier stress | Bounded ingress copies; Teredo/6to4 rejected; cancellation-aware resolver; typed redacted failures; no telemetry; source tests are not live-provider or app-runtime proof | Compose behind an explicit policy, then run sanctioned private-PKI/public-PKI and physical iOS/macOS tests before claiming DNS-rebinding resistance |
