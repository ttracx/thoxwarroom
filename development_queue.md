...
| WR-005 | 8.7 | App-wired read-only Hermes live review plus audited mutation coordinator seam implemented; actions remain disabled | Hermes + Security | `Packages/WarRoomHermes`; audited-operation contracts; Apple URLSession streaming transport; `App/HermesRunReview*` | Durable intent before transport; correlation replay rejection; no automatic retry; bounded outcome or explicit indeterminate state; read-only UI | 32 Hermes tests plus URLSession/app model tests; live/reconnect/crash-reconciliation evidence pending | No mutation controls or production durable-operation store; URLProtocol evidence is not live-provider proof | Implement production operation persistence/reconciliation, then capture live private-service evidence before approval UI |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI, SPDX generator, tracked-source secret scanner | Every PR regenerates deterministically, inventories dependencies, scans tracked source, tests every package with warnings as errors, and builds/tests both targets without release secrets | 174 package tests, 73 integrated macOS tests, 7 Python tests, deterministic assets/project, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; remote jobs stop before all steps with the account-lock annotation |
