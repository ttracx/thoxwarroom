...
| WR-003 | 8.8 | Profile encryption/migration, bounded encrypted audit ledger, and rollback anchor implemented; policy wiring pending | Core + Apple | `EncryptedWorkspaceData`; `EncryptedDurableAuditEventStore`; Apple AES-GCM/Keychain file store; encrypted onboarding migration | Profile/audit ciphertext isolation, redaction revalidation, internal tamper chain, Keychain head anchoring, idempotent append, bounded pages, and credential-first erasure pass | 29 Core + 55 Apple infrastructure tests; integrated app pipeline pass | Device-only keys, complete iOS protection, backup exclusion; no cross-instance/process CAS, raw UUID routing, retention/export, or app mutation wiring | Add cross-instance serialization, retention/export, and resumable deletion journal |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI | Every PR regenerates deterministically, tests every package with warnings as errors, and builds/tests both targets without release secrets | 136 package tests, 63 integrated macOS tests, deterministic assets/project, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; remote jobs stop before all steps with the account-lock annotation |
...
- Privacy manifest is implemented and bundle-verified; support/privacy metadata, SBOM, and signed provenance remain.
