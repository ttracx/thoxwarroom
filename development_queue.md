...
| WR-010 | 9.0 | Starts with WR-001 | Security/QA/Release | `.github/workflows`, scripts, manifests | Every PR builds/tests both targets; release produces signed TestFlight build and Developer ID signed, notarized DMG | Clean clone, clean device, upgrade, rollback, `spctl`, `stapler` | SBOM, privacy manifest, release checklist | Restore native CI without signing secrets on PRs |
| WR-011 | 8.0 | Ready | Core + platform | Current `WKWebView` compatibility shell | Require HTTPS, validate redirects, isolate data by workspace, implement sign-out purge | Navigation matrix and WKWebView integration tests | Document temporary cloud boundary | Harden shell while native slices are built |
| WR-012 | 7.2 | Ready | Release | Tool bootstrap and docs | One command installs/checks XcodeGen and builds a clean clone | Clean-host bootstrap smoke | README/build guide | Add pinned XcodeGen setup |
| WR-013 | 8.6 | Ready | iOS + Release | `Assets.xcassets`, icon generator, upload script | Asset catalog validates without icon warnings; documented ASC variables match script; signed archive export succeeds | `actool` validation and dry-run auth checks | No key material in repo/logs | Repair icons and ASC credential plumbing |
| WR-014 | 9.1 | Ready | macOS + Release | macOS build/package script | Developer ID signing; no `get-task-allow`; staple before final staging; final DMG app passes Gatekeeper | `codesign`, `stapler`, `spctl`, checksum | Release evidence record | Replace development-signing release path |
