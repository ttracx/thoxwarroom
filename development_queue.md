...
| WR-002 | 9.2 | Shared Core foundation implemented; UI/Keychain/transport pending | Core + Security | `Packages/WarRoomCore`; workspace profile, Keychain seam, transport policy | Validated local/private/hosted profile model; no hard-coded destination in core; explicit hosted opt-in; no network or secret persistence implementation yet | 16 package tests cover scheme/host/credentials/ports/boundaries, audit redaction, Codable round trips, request paths, and protocol substitutability | Package README, ADR-004, and security boundary updated | Build workspace onboarding and Keychain/transport adapters without weakening core policy |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI | Every PR regenerates deterministically and builds/tests both targets without release secrets | Repo-local XcodeGen checksum/version, asset/drift validation, 12 macOS tests, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; both remote attempts stopped before all steps with the account-lock annotation |
