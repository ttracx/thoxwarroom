...
| WR-002 | 9.2 | Core + native onboarding implemented; Keychain/transport pending | Core + Security + platform | `Packages/WarRoomCore`; `App/WorkspaceOnboarding*`; workspace profile, Keychain seam, transport policy | Validated local/private/hosted profile; explicit hosted opt-in; metadata-only persistence; exact-origin compatibility gate | 16 package tests plus 9 onboarding tests; integrated macOS 21/21 and generic iOS test build pass | README, ADR-004, security boundary, accessible privacy states | Implement scoped Keychain and URLSession adapters without weakening core policy |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI | Every PR regenerates deterministically and builds/tests both targets without release secrets | Repo-local XcodeGen checksum/version, asset/drift validation, 16 Shared Core tests, 21 macOS tests, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; four remote attempts, including `32639512911`, stopped before all steps with the account-lock annotation |
