...
| WR-013 | 8.6 | Implemented; ASC record pending | iOS + Release | `Assets.xcassets`, `App/PrivacyInfo.xcprivacy`, icon/privacy validators, upload script | Catalog and privacy manifest validate; documented ASC variables match; signed archive and IPA export succeed | Validators, direct `actool`, generic Simulator test build, archive/IPA manifest placement, and exported signature/provisioning verified | No tracking/collection declared; app-only UserDefaults `CA92.1`; key remains external with mode 600 | Create the App Store Connect app record for `ai.thox.warroom`, then rerun upload and verify processing/install |
