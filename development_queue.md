...
| WR-006 | 8.1 | Typed read-only Mesh adapter implemented; dashboard/live contract evidence pending | Hermes/War Room | `Packages/WarRoomMesh`; future fleet/route/alert adapters | Provenance, last-updated, stale/error/empty states against real endpoints | 13 synthetic adapter tests cover bounds, mesh isolation, freshness, timeout/error shapes | Device identifiers excluded from logs; fixtures synthetic | Capture current private Mesh gateway contract, then build the read-only dashboard |
...
| WR-010 | 9.0 | Implemented locally; GitHub billing blocked | Security/QA/Release | `.github/workflows`, pinned bootstrap, unsigned CI | Every PR regenerates deterministically, tests every package with warnings as errors, and builds/tests both targets without release secrets | 77 package tests, 43 macOS tests, deterministic assets/project, generic iOS Simulator test build all pass locally | Workflow has read-only contents permission and no signing/upload/notary operations | Unlock GitHub billing and rerun; remote jobs stop before all steps with the account-lock annotation |
