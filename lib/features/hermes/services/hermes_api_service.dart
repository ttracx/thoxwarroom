  ...
  void add(List<int> chunk) {
    ...
    if (_bytes > limits.maxBytes) {
      throw const HermesStreamGuardException(
        'The Hermes stream exceeded ThoxWarRoom\'s transfer limit.',
      );
    }
  ...
  void _finishLine() {
    if (!_lineHasBytes) {
      ...
      if (_frames > limits.maxRawFrames) {
        throw const HermesStreamGuardException(
          'The Hermes stream exceeded ThoxWarRoom\'s frame limit.',
        );
      }
    }
    _lineHasBytes = false;
  ...
  try {
    while (true) {
      ...
      if (remaining <= Duration.zero) {
        throw const HermesStreamGuardException(
          'The Hermes stream exceeded ThoxWarRoom\'s time limit.',
        );
      }
      ...
      if (events > limits.maxEvents) {
        throw const HermesStreamGuardException(
          'The Hermes stream exceeded ThoxWarRoom\'s event limit.',
        );
      }
      ...
      if (eventCharacters > remainingCharacters) {
        throw const HermesStreamGuardException(
          'The Hermes stream exceeded ThoxWarRoom\'s size limit.',
        );
      }
...
Future<Object?> _decodeHermesBoundedJsonValue(
  ...
  try {
    ...
    if (data is ResponseBody) {
      ...
      // depth, node, and aggregate-string ceilings before ThoxWarRoom uses it.
      decoded = data;
    }
    ...
    if (data is ResponseBody) {
      ...
      if (advertisedLength != null &&
          (advertisedLength > byteLimit ||
              advertisedLength > recoveryBudget.remainingBytes)) {
        throw const HermesStreamGuardException(
          'The Hermes response exceeded ThoxWarRoom\'s transfer limit.',
        );
      }
      ...
      try {
        ...
        throw HermesStreamGuardException(
          remaining <= idleTimeout
              ? 'The Hermes recovery response exceeded ThoxWarRoom\'s time limit.'
  ...
  try {
    ...
    throw switch (error.limit) {
      HermesJsonLimit.depth => const HermesStreamGuardException(
        'The Hermes recovery response exceeded ThoxWarRoom\'s nesting limit.',
      ),
      HermesJsonLimit.nodes => const HermesStreamGuardException(
        'The Hermes recovery response exceeded ThoxWarRoom\'s value limit.',
      ),
      HermesJsonLimit.tokens => const HermesStreamGuardException(
        'The Hermes recovery response exceeded ThoxWarRoom\'s token limit.',
      ),
    };
  }
}
  ...
  while (stack.isNotEmpty) {
    ...
    if (value is String) {
      ...
      if (characters > maxCharacters) {
        throw const HermesStreamGuardException(
          'The Hermes recovery response exceeded ThoxWarRoom\'s size limit.',
        );
      }
      continue;
    }
...
class HermesApiService {
      ...
      final response = await _dio
          ...
          .timeout(
            ...
            onTimeout: () {
              _signalHermesStreamCancellation(cancelToken);
              throw const HermesStreamGuardException(
                'The Hermes response exceeded ThoxWarRoom\'s time limit.',
              );
            },
          );
      recoveryBudget.requireRemainingDuration();
      ...
      final response = await _dio
          ...
          .timeout(
            ...
            onTimeout: () {
              ...
              throw const HermesStreamGuardException(
                'The Hermes create response exceeded ThoxWarRoom\'s time limit.',
              );
            },
          );
  ...
  Future<List<Map<String, dynamic>>> getSessionMessages(
    ...
    if (list.length > kMaxHermesSessionHistoryMessages) {
      _signalHermesStreamCancellation(requestCancelToken);
      throw const HermesStreamGuardException(
        'The Hermes session history exceeded ThoxWarRoom\'s message limit.',
      );
    }
...
List<Map<String, dynamic>> _boundedHermesMapList(
  ...
  if (candidate.length > kMaxHermesJsonCollectionItems) {
    throw const HermesStreamGuardException(
      'The Hermes response exceeded ThoxWarRoom\'s item limit.',
    );
  }
  return candidate
      .whereType<Map>()
