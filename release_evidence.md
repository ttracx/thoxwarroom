...
## 2026-08-23 DNS-bound HTTPS application-source checkpoint

- Functional application source is pushed on `main` through `af65551702b5b9ad0a74be17c87bb126926d44ae`, building on networking hardening commit `39aa7c7f8d9105c2bf68e7d6972f3dd2529674b9`. Every HTTPS Open WebUI, Hermes, and Mesh feature host now injects the reviewed DNS-bound transports. Explicitly validated loopback HTTP retains URLSession compatibility; decoded legacy private HTTP receives the HTTPS-only path and fails before credential injection; hosted HTTP fails endpoint decoding.
- The shared resolver binds each connection and redirect to a validated numeric snapshot while preserving original-host SNI and platform-default trust. The slice also enforces bounded HTTP/1.1 framing and ingress copies, exact-origin credential forwarding, cancellation-aware DNS, request/resource and stream-inactivity timeouts, Teredo/6to4 rejection, finite streaming buffers, typed redacted errors, and exactly-once terminal cancellation.
- Independent source/security review found no actionable issue after timeout and composition fixes. Current source-only validation passed 310 Swift package tests with warnings as errors, 7 Python tests, tracked-source secret scanning, privacy-manifest validation, app-icon validation, and a direct Swift compiler typecheck of all 32 app source files. Before composition, the hardened infrastructure package also passed 30 consecutive full runs (4,920 executions) after two flaky test seams were made deterministic.
- The next signed candidate must advance to version 4.2.0 build 7 because the preserved iOS build-6 archive was produced from older source `f44a3ade4570e5026c56bd84cd9cbf2189c7503b`. The project build number remains untouched at 6 during the shared-Xcode freeze and must be advanced/regenerated only after explicit release. Build 6's Organizer upload was last observed waiting for App Store Connect analysis; the receipt remains unread while shared Xcode is reserved by the MeshStack physical-acceptance lane. No build-6 upload completion, current-source artifact inclusion, macOS build-6 archive, or TestFlight processing is claimed here.
- Integrated Xcode tests, signed iOS/macOS build-7 archives, App Store Connect processing/group assignment, sanctioned private/public certificate flows, and physical TestFlight runtime evidence remain pending. The Xcode project/scheme/archive state was not touched during this source checkpoint.
