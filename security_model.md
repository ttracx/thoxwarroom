...
- The concrete URLSession transport enforces exact-origin redirects and local resource bounds but is not yet DNS-rebinding-resistant and has not completed live certificate/private-endpoint tests. A credential-free Network.framework spike proved numeric-peer connections can preserve original-host SNI/default trust on both deployment targets, including rejection of an expired certificate, but the app has not adopted that primitive because a bounded HTTP/1.1/SSE codec, redirect re-resolution, and race-safe cancellation coordinator are still required.
